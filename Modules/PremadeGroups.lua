...
local time = time
local LFGListCategorySelection_StartFindGroup, LFGListSearchPanel_SignUp =
	LFGListCategorySelection_StartFindGroup, LFGListSearchPanel_SignUp
...
function addon:OnPremadeGroupsEnable()
	...
	self:HookScript(LFGListFrame.CategorySelection, "OnShow", function()
		if PremadeGroups.dblClick.category then
...
function addon:LFGListApplicationDialog_OnShow(dialog)
	...
	if roles.current or (not roles.Tank and not roles.Healer and not roles.Damager) then
		...
		self:Print(
			string.format(
				L['Signed up for "%s (%s)" as current role: %s'],
				searchResultInfo.name,
				C_LFGList.GetActivityInfo(searchResultInfo.activityID),
				(
						(currentRole == "TANK" and L["tank"])
						or (currentRole == "HEALER" and L["healer"])
						or (currentRole == "DAMAGER" and L["damage"])
					)
			)
		)
	else
		...
		self:Print(
			string.format(
				L['Signed up for "%s (%s)" as roles: %s%s%s%s%s'],
				searchResultInfo.name,
				C_LFGList.GetActivityInfo(searchResultInfo.activityID),
				(roles.Tank and L["tank"] or ""),
				((roles.Tank and roles.Healer) and ", " or ""),
				(roles.Healer and L["healer"] or ""),
				(((roles.Tank or roles.Healer) and roles.Damager) and ", " or ""),
				(roles.Damager and L["damage"] or "")
			)
		)
	end
